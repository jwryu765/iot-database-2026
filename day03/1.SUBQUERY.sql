...
-- ANY, SOME 결과 중 메인쿼리이 조건이 하나라도 참이면 출력
-- 출판사별 가장비싼 책들과 가격이 하나라도 일치하는 책정보 조회하라
SELECT *
  FROM Book b 
...
-- 출판사별 가장비싼 책들과 가격이 하나라도 더 비싼 책정보 조회하라
SELECT *
  FROM Book b 
 WHERE price > ANY (SELECT max(price)
                      FROM Book
...
-- 구매 고객별 합계 중에서 총 금액이 3만2천 이상인 구매건의 고객번호와 금액를 조회하세요
