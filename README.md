...
- **BCNF(Boyce-Codd 정규형)**
    ...
    - (학생, 과목) -> 강사 결정가능
    - 과목만 가지고 강사를 결정가능. 학생은 강사 지정할 키가 아님
...
- 대량데이터 실습용 테이블 orders_big - [쿼리](./day08/3.대량데이터_인덱스_실습.sql)
- 순번 처리용 테이블 nums
...
```text
-> Sort: orders_big.order_date DESC  (cost=1.04e+6 rows=9.95e+6) (actual time=3282..3282 rows=26 loops=1)
    -> Filter: (orders_big.customer_id = 123456)  (cost=1.04e+6 rows=9.95e+6) (actual time=1421..3282 rows=26 loops=1)
        -> Table scan on orders_big  (cost=1.04e+6 rows=9.95e+6) (actual time=0.0745..2951 rows=10e+6 loops=1)
```
...
```text
-> Index lookup on orders_big using idx_orders_customer_id_and_order_date (customer_id=123456) (reverse)  (cost=26.4 rows=26) (actual time=0.0661..0.191 rows=26 loops=1)
```
...
- C학습 AddressBook 프로젝트와 비교 - [소스](./day09/MySQLCrudTest/main.cpp)
