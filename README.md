...
- SQL 종류
    - `Data Manipulation Language` - 데이터 조작어. `SELECT`, `INSERT`, `UPDATE`, `DELETE` 등 데이터를 조작하는 언어.
    - `Data Definition Language` - 데이더 정의어. `CREATE`, `ALTER`, RENAME, `DROP` 등 객체(데이터베이스, 테이블, 사용자, 뷰, 인덱스,.. )를 처리하는 언어.
    - `Data Control Language` - 데이터 제어어. `GRANT`, `REVOKE` 와 같이 사용자에게 권한주고 해제하는 기능을 처리하는 언어.
    - `Transaction Control Language` - 트랜잭션 제어어. `START TRANSACTION`, `COMMIT`, `ROLLBACK`, SAVEPOINT 등의 트랜잭션 처리로 동시성 제어를 위한 언어.
...
- CREATE 구문 - [쿼리](./day04/2.CREATE.sql)
    ...
    - FOREIGN KEY (custid) REFERENCES NewCustomer(custid) ON DELETE CASCADE,
        ...
        - ON UPDATE CASCADE | SET NULL : 수정할 때도 삭제시와 유사한 처리 가능. 수정도 가능하지만 PK 수정이 거의 없기때문에 많이 사용되지 않음
...
- ERD 모델링 툴
    - ERWin Data Modeler : 퀘스트 사에서 만든 대표적인 ERD 작성툴. 업계표준. 유로
    - eXERD : 한국산 모델링툴, 이클립스 기반. 유료
    - ER/Studio : 대규모 엔터프라이즈 데이터 모델링툴. 유로
    - Draw.io - https://app.diagrams.net/. 개념/논리 ERD 작성 가능. 기능 미흡. 무료
    - `erdcloud` - https://www.erdcloud.com/ 한국에서 개발한 웹기반 모델링들. 논리/물리 ERD 작성, 내보내기 기능, 무료/유료
...
- 개체(Entity)
    - 사람, 사물, 장소, 개념, 사전 등 유무형의 정보를 가진 독립적 실체
...
- 수강관리 시스템화 되기 이전 문제점
    - 이상현상
        ...
        - 수정 이상 : 박지은 강사의 전화번호가 바뀌면 Phython 과목을 듣는 모든 행을 찾아서 수정필요
...
- **제1정규형**(1NF)
    ...
    - 전화번호 컬럼에 010-2222-2222, 010-3333-3333 두 개의 값이 들어가면 단됨
...
- **제2정규형**(2NF)
    ...
    | P201 | Phython | 박지은 | 010-9999-2222 |
...
- 제3정규형(3NF)
    ...
    - 과목정보, 강사정보 분리, `이행적(함수) 종속성 제거`해야 함
    - 강사정보를 구분지을 수 있는 키속성이 생성되어야함
...
- **BCNF(Boyce-Codd 정규형)**
    - 모든 함수 종속 X -> Y에서 X는 반드시 `유일한 결정자`가 되어야함. PK를 제대로 결정하라
    - 키 종류 : 기본키, 외래키, 복합키(두 개의 속성이 기본키 구성), 후보키, 슈퍼키
    - 후보키 : 슈퍼키(기본키)가 될 수 있는 속성들
    - 슈퍼키 : 행을 유일하게 식별할 수 있는 속성 집합(한 개 또는 여러개). 유일하지만 하면 됨
...
```text
-> Sort: orders_big.order_date DESC  (cost=1.04e+6 rows=9.96e+6) (actual time=2565..2565 rows=26 loops=1)
    -> Filter: (orders_big.customer_id = 123456)  (cost=1.04e+6 rows=9.96e+6) (actual time=1144..2565 rows=26 loops=1)
        -> Table scan on orders_big  (cost=1.04e+6 rows=9.96e+6) (actual time=0.0862..2274 rows=10e+6 loops=1)
```
...
```test
-> Index lookup on orders_big using idx_orders_customer_id_and_order_date (customer_id=123456) (reverse)  (cost=26.5 rows=26) (actual time=0.769..1.93 rows=26 loops=1)
```
